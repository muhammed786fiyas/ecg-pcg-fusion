"""Train cross_attn_resnet18_pcgpre.

cross_attn_resnet18, with its PCG branch initialised from the ResNet-18 PCG branch
...
schedule, seed, head - is byte-identical to cross_attn_resnet18, so the pair isolates the
...
MODEL_FAMILY = "cross_attn_resnet18_pcgpre"
UINT8_MAX = 255.0
IMAGE_SIZE = 224
...
class BidirectionalCrossAttention(nn.Module):
    ...
    def forward(self, ecg_tokens, pcg_tokens):
        ...
        return (
            self.ecg_norm(ecg_tokens + attended_ecg),
            self.pcg_norm(pcg_tokens + attended_pcg),
        )


class ResNetBranch(nn.Module):
    """Pretrained ResNet-18 with the final fc removed.
    ...
    512-d avgpool output is projected to 256 so the cross-attention block is
    identical to the custom-CNN version.
    """
...
class CrossAttnResNet18(nn.Module):
    def __init__(self, dropout, n_heads, head_dim, proj_dim):
        super().__init__()
        self.ecg_branch = ResNetBranch(proj_dim)
        self.pcg_branch = ResNetBranch(proj_dim)
        self.cross_attention = BidirectionalCrossAttention(proj_dim, n_heads, head_dim)
...
def build_model(params):
    family = params["modeling"]["cross_attn_resnet18_pcgpre"]
    return CrossAttnResNet18(
