...
class EcgOnly(nn.Module):
    """One CNN branch on the ECG scalogram. The ablation floor for fusion.

    Takes both tensors and ignores the PCG one so the Dataset and the epoch loop
    stay identical to the fusion scripts - the only thing that differs between
    the unimodal baselines and dual_cnn is the model.
    """
...
def build_model(params):
    return EcgOnly(params["modeling"]["common"]["dropout"])
