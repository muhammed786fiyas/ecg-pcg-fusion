...
class PcgOnly(nn.Module):
    """One CNN branch on the PCG scalogram. The ablation floor for fusion.

    Takes both tensors and ignores the ECG one so the Dataset and the epoch loop
    stay identical to the fusion scripts.
    """
...
def build_model(params):
    return PcgOnly(params["modeling"]["common"]["dropout"])
