"""Train resnet18_ecg_only.

The ResNet-18 unimodal baseline: one pretrained ResNet-18 branch on the ECG
...
MODEL_FAMILY = "resnet18_ecg_only"
UINT8_MAX = 255.0
IMAGE_SIZE = 224
...
class ResNet18EcgOnly(nn.Module):
    """One pretrained ResNet-18 branch on the ECG scalogram.
    ...
    schedule and seed; the only things removed are the PCG branch and
    the cross-attention. Takes both tensors and ignores the PCG one so
    ...
    def forward(self, ecg, pcg):
        return self.classifier(self.ecg_branch(ecg)).squeeze(1)


def build_model(params):
    family = params["modeling"]["resnet18_ecg_only"]
    return ResNet18EcgOnly(params["modeling"]["common"]["dropout"], family["proj_dim"])
