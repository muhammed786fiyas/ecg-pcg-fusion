...
MODEL_FAMILY = "cross_attn_fusion"
UINT8_MAX = 255.0
IMAGE_SIZE = 224
...
class BidirectionalCrossAttention(nn.Module):
    ...
    Each branch's 14x14x256 map flattens to 196 tokens of dim 256. One layer in
    each direction is enough - this is a fusion block, not a deep transformer.
    """
...
class CrossAttnFusion(nn.Module):
    """The headline model. Bidirectional cross-modal attention over the two
    branches' token sequences, mean-pooled and concatenated."""

    def __init__(self, dropout, n_heads, head_dim):
        super().__init__()
        self.ecg_branch = CNNBranch()
        self.pcg_branch = CNNBranch()
        self.cross_attention = BidirectionalCrossAttention(256, n_heads, head_dim)
...
def build_model(params):
    family = params["modeling"]["cross_attn_fusion"]
    return CrossAttnFusion(
        params["modeling"]["common"]["dropout"], family["n_heads"], family["head_dim"]
    )
